...
@section('content')
    ...
    <form action="{{ route('shopping-carts.store') }}" method="POST" id="carrito-form"
        x-data="carritoForm()"
        x-init="initForm()"
        @submit="loading = true">
        @csrf
        ...
        <div class="bg-white dark:bg-zinc-800 shadow rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="CLI_Ced_Ruc" id="cliente-select" class="w-full rounded-md border-gray-300 dark:bg-zinc-900 dark:text-white" required>
                        <option value="">Seleccione Cliente</option>
                        @foreach ($clientes as $cliente)
                        <option value="{{ $cliente->CLI_Ced_Ruc }}">{{ $cliente->CLI_Ced_Ruc }} - {{ $cliente->CLI_Nombre }} {{ $cliente->CLI_Apellido }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
