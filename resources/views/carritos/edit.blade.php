...
@section('content')
    ...
    <form action="{{ route('shopping-carts.update', $carrito->CRC_Carrito) }}" method="POST" id="carrito-form"
        x-data="carritoFormEdit()"
        x-init="initForm({{ $carrito->productos->map(function($p) { return ['id' => uniqid(), 'code' => $p->PRO_Codigo, 'qty' => $p->pivot->CRD_Cantidad, 'price' => $p->PRO_Precio]; }) }})"
        @submit="loading = true">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-zinc-800 shadow rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="cliente-select" class="w-full rounded-md border-gray-300 bg-gray-100 dark:bg-zinc-800 dark:text-gray-400" disabled>
                        @foreach ($clientes as $cliente)
                        <option value="{{ $cliente->CLI_Ced_Ruc }}" {{ $carrito->CLI_Ced_Ruc == $cliente->CLI_Ced_Ruc ? 'selected' : '' }}>
        ...
        <div class="bg-white dark:bg-zinc-800 shadow rounded-lg p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <button type="button" @click="addRow()"
                    class="bg-green-600 text-white px-3 py-1 rounded-md text-sm hover:bg-green-700">+ Agregar Producto</button>
            </div>
                    ...
                    <template x-for="(row, index) in rows" :key="row.id">
                        <tr class="border-b dark:border-zinc-700">
                            ...
                            <td class="py-3 text-center">
                                <button type="button" @click="removeRow(row.id)" class="text-red-500 hover:text-red-700">✕</button>
                            </td>
                        </tr>
                    </template>
                </tbody>
        ...
        Alpine.data('carritoFormEdit', () => ({
            ...
            initSelect2(el, row) {
                ...
                $(el).on('select2:select', (e) => {
                    ...
                    if (isDuplicate) {
                        this.showError("Este producto ya está en el carrito. Ajuste la cantidad en su lugar.");
