...
@section('content')
    ...
    <form action="{{ route('purchase-orders.store') }}" method="POST" id="order-form" 
        x-data="ordenForm()"
        x-init="initForm()"
        @submit="loading = true">
        @csrf
                ...
                <tbody id="items-body">
                    <template x-for="(row, index) in rows" :key="row.id">
                        <tr class="border-b dark:border-zinc-700">
                            ...
                            <td class="py-3">
                                <input type="number" step="0.01" class="w-full border-none bg-transparent font-semibold dark:text-white" 
