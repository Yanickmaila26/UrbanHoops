...
@section('content')
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            Gestiona las subcategorías y su vinculación con categorías
                        </p>
                    </div>
            ...
            <div class="mb-6">
                <div class="bg-white dark:bg-zinc-800 rounded-lg shadow p-4">
                    <form action="{{ route('subcategories.index') }}" method="GET" class="space-y-4"
    ...
    <div class="mb-4">
        <p class="text-sm text-gray-600 dark:text-gray-400">
            Total de subcategorías: <span class="font-semibold">{{ $subcategorias->total() }}</span>
            @if ($search)
                | Resultados encontrados: <span class="font-semibold">{{ $subcategorias->count() }}</span>
            @endif
        </p>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-800 overflow-hidden shadow rounded-lg">
                    ...
                    @forelse ($subcategorias as $subcategoria)
                        <tr class="hover:bg-gray-50 dark:hover:bg-zinc-700 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">
                                {{ $subcategoria->SCT_Codigo }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                {{ $subcategoria->SCT_Nombre }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                {{ $subcategoria->categoria->CAT_Nombre ?? 'Sin categoría' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-3">
                                    <a href="{{ route('subcategories.edit', $subcategoria->SCT_Codigo) }}"
                                    ...
                                    <button onclick="openDeleteModal('{{ $subcategoria->SCT_Codigo }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                                ...
                                <a href="{{ route('subcategories.create') }}" class="text-blue-600 underline">Crear
                                    una
                                    nueva</a>
                            </td>
                        </tr>
        ...
        @if (method_exists($subcategorias, 'links'))
        ...
        <div class="flex items-center justify-center min-h-screen px-4 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <form id="delete-form" method="POST" class="p-6" x-data="{ deleting: false }" @submit="deleting = true">
                    @csrf
                    @method('DELETE')
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">Eliminar Subcategoría</h3>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">¿Estás seguro de eliminar esta subcategoría?
                        Los productos asociados quedarán sin categoría.</p>
    ...
    <script>
        function openDeleteModal(codigo) {
            ...
            form.action = `/admin/subcategorias/${codigo}`;
